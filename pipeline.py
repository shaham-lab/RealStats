...
parser.add_argument('--dataset_type', type=str, default='DEEPFAKE_BENCHMARK', choices=[e.name for e in DatasetType], help='Type of dataset configuration to use.')
...
parser.add_argument('--statistics', type=str, nargs='+', default=[k for k in STATISTIC_HISTOGRAMS if (k.startswith("RIGID.DINO.") or k.startswith("RIGID.CLIPOPENAI.")) and any(k.endswith(suffix) for suffix in [".05", ".10"])])
